...
class HotDog {
  public:
	...
	HotDog(int id, std::shared_ptr<Sausage> sausage, std::shared_ptr<Bread> bread)
		...
		// std::cout << std::fixed << std::setprecision(6) << "Sasauge (" << id_ << ") time "
		// 		  << std::chrono::duration<double>(sausage_->GetCookDuration().count()) / 1000000000 << std::endl;
		// std::cout << std::fixed << std::setprecision(6) << "Bread (" << id_ << ") time "
		// 		  << std::chrono::duration<double>(bread_->GetBakingDuration().count()) / 1000000000 << std::endl;
