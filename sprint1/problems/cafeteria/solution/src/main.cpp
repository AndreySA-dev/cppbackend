...
void PrintHotDogResult(const Result<HotDog>& result, Clock::duration order_duration) {
	...
	auto as_seconds = [](auto d) { return std::chrono::duration_cast<std::chrono::duration<double>>(d).count(); };
	...
	if (result.HasValue()) {
		...
		os << "Hot dog #" << hot_dog.GetId()
		   ...
		   << "s, sausage bake time: " << as_seconds(hot_dog.GetSausage().GetCookDuration()) << "s" << std::endl;
	} else {
		try {
			result.ThrowIfHoldsError();
		} catch (const std::exception& e) { os << "Error: " << e.what() << std::endl; } catch (...) {
...
std::vector<HotDog> PrepareHotDogs(int num_orders, unsigned num_threads) {
	...
	for (int i = 0; i < num_orders; ++i) {
		...
		net::dispatch(io, [&cafeteria, &hotdogs, &mut, i, start_time, &start, num_waiting_threads] {
			std::osyncstream{std::cout} << "Order #" << i << " is scheduled on thread #" << std::this_thread::get_id()
										<< std::endl;
...
int main() {
	...
	std::cout << "Size - " << hotdogs.size() << std::endl;
